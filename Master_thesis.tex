...
Let us consider body B of Fig. (\ref{fig:Damage2}) and take a representative volume element(RVE) at an arbitrary point $P(x)$ in B. If the total void area in $dA$ is  $ dA_{D} $, the mechanical effect of $dA$ will be decreased by $dA_{D}$. Then the area,
\begin{equation}
  d\tilde{A} = dA  - dA_{D}
  \label{eqn:da} 
\end{equation}
...
\begin{figure}[htbp!]
\begin{center}
\includegraphics[width=0.6\textwidth]{{22.Convergence_error.png}}
...
\end{figure}
\FloatBarrier
\indent\indent\indent  The force-displacement response for the three different meshes with and without the regularization (Section(\ref{Mesh Regularisation})) schemes have been plotted in the Figure (\ref{fig:with regularization})  and (\ref{fig:without regularization}) respectively. The global-force displacement response is characterized by a large linear regime followed by a gradual drop of load (because of the softening) or sudden drop in case of no regularization. From figure (\ref{fig:Convergence study}) it is clearly evident that the model with regularization gives stable force-displacement response  compared to the model without regularization where the maximum load capacity is very low and the the load drops significantly with decrease in mesh size(or increase in number of elements). Even though the regularization scheme does not fully alleviate the mesh dependency, it improves the solution significantly compared to the material model without regularization. 
\begin{figure}[htbp!]
     \captionsetup[subfigure]{justification=centering}
...
\end{figure}
\FloatBarrier
\indent\indent\indent From the Figure (\ref{fig:Stress_Comp}), it is clear that the increase in shear contribution has little to no effect in the initiation and distribution of the damage $d_{1}$ because the shear stress component ($\sigma_{12}$) is very low compared to the stress in longitudinal direction ($\sigma_{11}$). But the shear stress ($\sigma_{12}$) affects the damage initiation and propagation of damage $d_{2}$ because its magnitude is almost similar to the stress component ($\sigma_{22}$) which was the only component responsible for damage $d_{2}$ in case of the Maximum stress criterion previously utilized. 
\begin{figure}[htbp!]
\begin{center}
...
\end{figure}
\FloatBarrier
\indent\indent\indent In case of transverse loading the damage  $d_{2}$ is unaffected because the magnitude of the shear stress ($\sigma_{12}$) is very low compared to the stress ($\sigma_{22}$). But the damage $d_{1}$ is affected because the magnitude of the shear stress component ($\sigma_{12}$) is similar compared to ($\sigma_{11}$).
\begin{figure}[htbp!]
\begin{center}
